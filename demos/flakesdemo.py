...
# SPARK(pyspk) Flakes Demo script
...
class App:
    def __init__(self, width, height):
        ...
        renderer.enableRenderingHint(spk.DEPTH_TEST, False)  # notifying not to use depth info
        ...
        group = spk.Group.create(model, self.NB_PARTICLES[-1])  # type: spk.Group
            ...
            if event.type == sdl2.SDL_KEYDOWN and \
               (event.key.keysym.sym == sdl2.SDLK_PLUS or event.key.keysym.sym == sdl2.SDLK_KP_PLUS):
                if self.particleIndex < len(self.NB_PARTICLES) - 1:
                    ...
                    self.group.addParticles(nb_current - nb_before, zone=self.sphere, velocity=spk.Vector3D())
        ...
        if not self.paused:
            ...
            if self.system.getNbParticles() == 0:
                self.group.addParticles(self.NB_PARTICLES[self.particleIndex],
                                        zone=self.obstacle.getZone(), velocity=spk.Vector3D())
